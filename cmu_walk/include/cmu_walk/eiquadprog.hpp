...
inline void compute_d(Matrix<double,nVars,1> & d, 
    const Matrix<double,nVars,nVars> & J, const Matrix<double,nVars,1> & np)
{
  d = J.adjoint() * np;
}

template<int nVars, int nEqCon, int nIneqCon>
inline void update_z(Matrix<double,nVars,1> & z, 
    const Matrix<double,nVars,nVars> & J, 
    const Matrix<double,nVars,1> & d, int iq)
{
  z = J.rightCols(z.size()-iq) * d.tail(d.size()-iq);
}

template<int nVars, int nEqCon, int nIneqCon>
inline void update_r(const Matrix<double,nVars,nVars> & R, 
    Matrix<double,nIneqCon+nEqCon,1>& r, const Matrix<double,nVars,1>& d, 
    int iq) 
{
...
inline bool add_constraint(
    Matrix<double,nVars,nVars> & R, Matrix<double,nVars,nVars> & J, 
    Matrix<double,nVars,1> & d, 
...
inline void delete_constraint(
    Matrix<double,nVars,nVars>& R, Matrix<double,nVars,nVars>& J, 
    Matrix<int,nIneqCon+nEqCon,1> & A, Matrix<double,nIneqCon+nEqCon,1> & u,  
    int p, int& iq, int l)
{
...
template<int nVars, int nEqCon, int nIneqCon>
inline double solve_quadprog2(
    LLT<Matrix<double,nVars,nVars>,Lower> & chol,  double c1, 
    Matrix<double,nVars,1> & g0,  
    const Matrix<double,nVars,nEqCon> & CE, const Matrix<double,nEqCon,1> & ce0,  
    const Matrix<double,nVars,nIneqCon> & CI, const Matrix<double,nIneqCon,1> & ci0, 
    Matrix<double,nVars,1> & x)
{
  int i, j, k, l; /* indices */
  int ip, me, mi;
  int n=nVars;
  int p=nEqCon; 
  int m=nIneqCon;
  Matrix<double,nVars,nVars> R, J;
  
  Matrix<double,nIneqCon+nEqCon,1> s, r, u, u_old; 
  Matrix<double,nVars,1> z, d, np, x_old;
  ...
  Matrix<int,nIneqCon+nEqCon,1> A, A_old, iai, iaexcl;
  int q;
  int iq, iter = 0;
 ...
 print_matrix("J", J, n);
  ...
  iq = 0;
    ...
    np = CE.col(i);
    compute_d<nVars,nEqCon,nIneqCon>(d, J, np);
		update_z<nVars,nEqCon,nIneqCon>(z, J, d,  iq);
		update_r<nVars,nEqCon,nIneqCon>(R, r, d,  iq);
#ifdef TRACE_SOLVER
		print_matrix("R", R, iq);
		print_vector("z", z, n);
		print_vector("r", r, iq);
    ...
    if (!add_constraint<nVars,nEqCon,nIneqCon>(R, J, d, iq, R_norm))
    {
...
l2a:/* Step 2a: determine step direction */
  ...
  if (t2 >= inf)
  {
    ...
    u(iq) += t;
    iai(l) = l;
    delete_constraint<nVars,nEqCon,nIneqCon>(R, J, A, u, p, iq, l);
#ifdef TRACE_SOLVER
  ...
  if (t == t2)
  {
    ...
    /* add constraint ip to the active set*/
		...
		{
			iaexcl(ip) = 0;
			delete_constraint<nVars,nEqCon,nIneqCon>(R, J, A, u, p, iq, ip);
#ifdef TRACE_SOLVER
      print_matrix("R", R, n);
...
inline double solve_quadprog(
    Matrix<double,nVars,nVars> & G, Matrix<double,nVars,1> & g0,  
    const Matrix<double,nVars,nEqCon> & CE, const Matrix<double,nEqCon,1> & ce0,  
    const Matrix<double,nVars,nIneqCon> & CI, const Matrix<double,nIneqCon,1> & ci0, 
    Matrix<double,nVars,1> & x)
{
						  
  LLT<Matrix<double,nVars,nVars>,Lower> chol;
  double c1;
  ...
  return solve_quadprog2<nVars,nEqCon,nIneqCon>(chol, c1, 
             g0, CE, ce0, CI, ci0, x);
}
