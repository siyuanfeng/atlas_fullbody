...
bool HatIdCon::ID(const RobotState &rs, const IDcmd &cmd, Command &output)
{ 
  // different model
  if (rs.getType() != _type)
    return false;
  ...
  for (int side = 0; side < LR; side++)
  	rs.feet[side].getBoundingBox(cmd.footRef[side], sup_box[side]);
  ...
  int nVars = HAT_N_SDFAST_U + HAT_N_JOINTS + nForces;
  int nEqCon = HAT_N_SDFAST_U;
  int nIneqCon = 9*nForces/6;
  const int forceStart = HAT_N_SDFAST_U + HAT_N_JOINTS;
  const int torqueStart = HAT_N_SDFAST_U;
  ...
  for (int side = 0; side < LR; side++) {
    ...
    Eigen::Quaterniond q = rs.feet[side].w_q;
    //Eigen::Quaterniond q = onlyYaw(rs.feet[side].w_q);
  ...
  switch (rs.contactState) {
    ...
    case DSr:
    case DSc:
      Rot.block<6,6>(0,0) = Rot_foot[LEFT];
      Rot.block<6,6>(6,6) = Rot_foot[RIGHT]; 
      break;
    case SSL:
      Rot.block<6,6>(0,0) = Rot_foot[LEFT];
      break;
    case SSR:
      Rot.block<6,6>(0,0) = Rot_foot[RIGHT]; 
      break;
    default:
      assert(0);
  }
  ...
  // Foot jacobian 
  const Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> *Jf[2];
  const Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> *Jfd[2];
  for (int side = 0; side < LR; side++) {
    switch (cmd.footRef[side]) {
      case Foot::RefCenter:
        Jf[side] = &(rs.Jmid[side]);
        Jfd[side] = &(rs.Jmidd[side]);
        break;

      case Foot::RefToe:
        Jf[side] = &(rs.Jtoe[side]);
        Jfd[side] = &(rs.Jtoed[side]);
        break;
        
      case Foot::RefHeel:
        Jf[side] = &(rs.Jheel[side]);
        Jfd[side] = &(rs.Jheeld[side]);
        break;
  ...
  // qdot
  Eigen::Matrix<double,HAT_N_SDFAST_U,1> qdot;
  qdot << Eigen::Map<const Eigen::Matrix<double,6,1> >(rs.rootd),
          Eigen::Map<const Eigen::Matrix<double,HAT_N_JOINTS,1> >(rs.jointsd);
  qdot *= QD_JD_USE_FRAC;
  ...
  */
  ...
  A.block<3,HAT_N_SDFAST_U>(rowIdx,0) = rs.Jpelvis.block<HAT_N_SDFAST_U,3>(0,3).transpose() * ANGDD_P_WEIGHT;
  b.segment<3>(rowIdx) += -rs.Jpelvisd.block<HAT_N_SDFAST_U,3>(0,3).transpose()*qdot;
   ...
   * bD = kp(q* - q) + kd(qd* - qd);
   ****************************/
  Eigen::Matrix<double,HAT_N_JOINTS,1> tmpjdd;
  HatRobotState::pelv2hat(cmd.jointsdd, tmpjdd.data());
  b.segment<HAT_N_JOINTS>(rowIdx) = tmpjdd;
  int pelvIdx;
  for (int i = 0; i < HAT_N_JOINTS; i++) {
    if (b(rowIdx+i) == std::numeric_limits<double>::infinity()) {
      ...
      //pelvIdx = HatRobotState::hatIdx2pelvIdx(i);
  ...
  A.block(rowIdx,6,HAT_N_JOINTS,HAT_N_JOINTS) *= QDD_REG_WEIGHT;
  b.segment<HAT_N_JOINTS>(rowIdx) *= QDD_REG_WEIGHT;
  ...
  {
    A.block<3,HAT_N_SDFAST_U>(rowIdx,0) = rs.Jc.transpose();
    ...
    A.block<3,HAT_N_SDFAST_U>(rowIdx,0) *= COMDD_XY_WEIGHT;
  ...
  {
    A.block<3,HAT_N_SDFAST_U>(rowIdx,0) = rs.Jtorso.block<HAT_N_SDFAST_U,3>(0,3).transpose() * ANGDD_T_WEIGHT;
    b.segment<3>(rowIdx) += -rs.Jtorsod.block<HAT_N_SDFAST_U,3>(0,3).transpose()*qdot;
  ...
  for (int side = 0; side < LR; side ++) {
    ...
    b.segment<6>(rowIdx) = Eigen::Map<const Eigen::Matrix<double,6,1> >(cmd.footdd[side]);
    A.block<6,HAT_N_SDFAST_U>(rowIdx,0) = Jf[side]->transpose() * FOOTDD_WEIGHT;
    b.segment<6>(rowIdx) += -Jf[side]->transpose()*qdot;
    b.segment<6>(rowIdx) *= FOOTDD_WEIGHT;

    // stance
    if (!isSwing(rs.contactState, side)) {
      A.block<6,HAT_N_SDFAST_U>(rowIdx,0) *= 5e1;
   ...
   * bT = 0;
   ****************************/
  A.block<HAT_N_JOINTS,HAT_N_JOINTS>(rowIdx,torqueStart).setIdentity();

  A.block<HAT_N_JOINTS,HAT_N_JOINTS>(rowIdx,torqueStart) *= TAU_REG_WEIGHT;
  b.segment<HAT_N_JOINTS>(rowIdx) *= TAU_REG_WEIGHT;
  assert(!(b.segment<HAT_N_JOINTS>(rowIdx).array() == std::numeric_limits<double>::infinity()).any());
  rowIdx += HAT_N_JOINTS;
  ...
  //if (DTAU_REG_WEIGHT != 0 && !_torqueOld.isZero()) {
    A.block<HAT_N_JOINTS,HAT_N_JOINTS>(rowIdx,torqueStart).setIdentity();
    b.segment<HAT_N_JOINTS>(rowIdx) = _torqueOld.segment<HAT_N_JOINTS>(0);
  
    A.block<HAT_N_JOINTS,HAT_N_JOINTS>(rowIdx,torqueStart) *= DTAU_REG_WEIGHT;
    b.segment<HAT_N_JOINTS>(rowIdx) *= DTAU_REG_WEIGHT;
    assert(!(b.segment<HAT_N_JOINTS>(rowIdx).array() == std::numeric_limits<double>::infinity()).any());
    rowIdx += HAT_N_JOINTS;
  }
  ...
  ***********************************************/
   ...
   * bD = tau
   ****************************/
  Eigen::Map<Eigen::MatrixXd> AD(_AD.data(),HAT_N_SDFAST_U,nVars);
  AD.topLeftCorner<HAT_N_SDFAST_U,HAT_N_SDFAST_U>() = rs.M;
  AD.block<HAT_N_SDFAST_U,HAT_N_JOINTS>(0,HAT_N_SDFAST_U) <<
      Eigen::Matrix<double,6,HAT_N_JOINTS>::Zero(),
      -Eigen::Matrix<double,HAT_N_JOINTS,HAT_N_JOINTS>::Identity();
  switch (rs.contactState) {
    case DSl:
    case DSr:
    case DSc: AD.rightCols<12>() << -(*Jf[LEFT]), -(*Jf[RIGHT]); break;
    case SSL: AD.rightCols<6>() = -(*Jf[LEFT]);  break;
    case SSR: AD.rightCols<6>() = -(*Jf[RIGHT]); break;
    default: assert(0); // BUG!
  }
  //Eigen::Map<const Eigen::Matrix<double,HAT_N_SDFAST_U,1> > bD(rs.nonLin);
  Eigen::Matrix<double,HAT_N_SDFAST_U,1> bD = rs.nonLin;
  bD.segment<6>(0) += _magicFT;
  bD += rs.Jtorso * _magicFTtorso;
  ...
  Eigen::Matrix<double,2*HAT_N_JOINTS,HAT_N_JOINTS> TauLim;
  TauLim.block<HAT_N_JOINTS,HAT_N_JOINTS>(0,0) = Eigen::Matrix<double,HAT_N_JOINTS,HAT_N_JOINTS>::Identity();
  TauLim.block<HAT_N_JOINTS,HAT_N_JOINTS>(HAT_N_JOINTS,0) = -Eigen::Matrix<double,HAT_N_JOINTS,HAT_N_JOINTS>::Identity();
  
  Eigen::Matrix<double,2*HAT_N_JOINTS,1> tauLim;
  for (int i = 0; i < HAT_N_JOINTS; i++) {
    int j = HatRobotState::hatIdx2pelvIdx(i);
    Transmission::getTorqueLimit(j, rs.joints[j], tauLim.data()+i, tauLim.data()+i+HAT_N_JOINTS);
  }

  tauLim.segment<HAT_N_JOINTS>(0) *= -1;

  // toeoff kny acc and uay acc go pos
  Eigen::Matrix<double,4,HAT_N_SDFAST_U> AccLim;
  Eigen::Matrix<double,4,1> accLim;
  AccLim.setZero();
  accLim.setZero();
  // left
  if (cmd.footRef[LEFT] == Foot::RefToe) {
    AccLim(0, HAT_S_L_LEG_KNY) = 1;
    AccLim(1, HAT_S_L_LEG_UAY) = 1;
    accLim(0) = -0;
    accLim(1) = -0;
  }
  // right
  if (cmd.footRef[RIGHT] == Foot::RefToe) {
    AccLim(2, HAT_S_R_LEG_KNY) = 1;
    AccLim(3, HAT_S_R_LEG_UAY) = 1;
    accLim(2) = -0;
    accLim(3) = -0;
  }
  ...
  */ 
  ...
  switch (rs.contactState) {
    ...
    case DSr:
    case DSc: {
      const int N_VARS = HAT_N_SDFAST_U + HAT_N_JOINTS + 12;
      const int N_EQ_CON = HAT_N_SDFAST_U;
      //const int N_INEQ_CON = 9*12/6;
      const int N_INEQ_CON = 9*12/6 + HAT_N_JOINTS*2 + 4;
      ...
      CI.setZero();
      CI.block<2*HAT_N_JOINTS,HAT_N_JOINTS>(0,torqueStart) = TauLim;
      CI.block<9*12/6,12>(2*HAT_N_JOINTS,forceStart) = AF;
      CI.block<4,HAT_N_SDFAST_U>(2*HAT_N_JOINTS+9*12/6,0) = AccLim;
      
      Eigen::Matrix<double,N_INEQ_CON,1> ci0;
      ci0.setZero();
      ci0.segment<HAT_N_JOINTS*2>(0) = tauLim;
      ci0.segment<4>(2*HAT_N_JOINTS+9*12/6) = accLim;
    ...
    case SSL: 
    case SSR: {
      const int N_VARS = HAT_N_SDFAST_U + HAT_N_JOINTS + 6;
      const int N_EQ_CON = HAT_N_SDFAST_U;
      //const int N_INEQ_CON = 9*6/6;
      const int N_INEQ_CON = 9*6/6 + 2*HAT_N_JOINTS + 4;
      Eigen::Matrix<double,N_VARS,N_VARS> G = 
          A.topRows(rowIdx).transpose()*A.topRows(rowIdx);
      Eigen::Matrix<double,N_VARS,1> g0 = 
          -A.topRows(rowIdx).transpose()*b.topRows(rowIdx);
      ...
      CI.setZero();
      CI.block<2*HAT_N_JOINTS,HAT_N_JOINTS>(0,torqueStart) = TauLim;
      CI.block<9*6/6,6>(2*HAT_N_JOINTS,forceStart) = AF; 
      CI.block<4,HAT_N_SDFAST_U>(2*HAT_N_JOINTS+9*6/6,0) = AccLim;
      
      Eigen::Matrix<double,N_INEQ_CON,1> ci0;
      ci0.setZero();
      ci0.segment<HAT_N_JOINTS*2>(0) = tauLim;
      ci0.segment<4>(2*HAT_N_JOINTS+9*6/6) = accLim;
  ...
  // save old torque
  _torqueOld.segment<HAT_N_JOINTS>(0) = X.segment<HAT_N_JOINTS>(torqueStart);
  ...
  /////////////////////////////////////////////////////////////
  // process output
  Eigen::Matrix<double,HAT_N_SDFAST_U,1> id_acc = X.topRows(HAT_N_SDFAST_U);
  Eigen::Matrix<double,HAT_N_JOINTS,1> id_trq = X.segment<HAT_N_JOINTS>(torqueStart);
  ...
  HatRobotState::hat2pelv(id_acc.data()+6, output.jointsdd_d);
  HatRobotState::hat2pelv(id_trq.data(), output.trq_ff);
  ...
  id_torsoWd = rs.Jtorso.block<HAT_N_SDFAST_U,3>(0,3).transpose() * id_acc - rs.Jtorsod.block<HAT_N_SDFAST_U,3>(0,3).transpose()*qdot;
  
  Eigen::Map <Eigen::Vector3d> id_pelvWd(output.pelvWd_d);
  id_pelvWd = rs.Jpelvis.block<HAT_N_SDFAST_U,3>(0,3).transpose() * id_acc - rs.Jpelvisd.block<HAT_N_SDFAST_U,3>(0,3).transpose()*qdot;
  
  Eigen::Map <Eigen::Matrix<double,6,1> > id_footdd_l(output.footdd_d[LEFT]);
  id_footdd_l = Jf[LEFT]->transpose() * id_acc - Jfd[LEFT]->transpose()*qdot;

  Eigen::Map <Eigen::Matrix<double,6,1> > id_footdd_r(output.footdd_d[RIGHT]);
  id_footdd_r = Jf[RIGHT]->transpose() * id_acc - Jfd[RIGHT]->transpose()*qdot;
  ...
  if (ID_IS_MAGIC_INT_T_TORQUE_ON(integrateMagicMask)) 
    _magicFTtorso.segment<3>(3) -= MAGIC_TORSO_TRQ_GAIN*id_torsoWd;
...
void HatIdCon::addToLog(BatchLogger &logger) 
{
	...
	logger.add_datapoint("ID.magicTau[Z]","a", &(_magicFT.data()[ZZ+3]));
 	...
 	logger.add_datapoint("ID.magicTorsoF[X]","a", &(_magicFTtorso.data()[XX]));
	logger.add_datapoint("ID.magicTorsoF[Y]","a", &(_magicFTtorso.data()[YY]));
	logger.add_datapoint("ID.magicTorsoF[Z]","a", &(_magicFTtorso.data()[ZZ]));
	logger.add_datapoint("ID.magicTorsoTau[X]","a", &(_magicFTtorso.data()[XX+3]));
	logger.add_datapoint("ID.magicTorsoTau[Y]","a", &(_magicFTtorso.data()[YY+3]));
	logger.add_datapoint("ID.magicTorsoTau[Z]","a", &(_magicFTtorso.data()[ZZ+3])); 
