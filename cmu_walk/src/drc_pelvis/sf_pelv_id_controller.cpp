...
bool SFPelvIdCon::ID(const RobotState &rs, const IDcmd &cmd, Command &output)
{ 
  // different model
  if (rs.getType() != _type)
    return false;
  ...
  int nVars = PELV_N_SDFAST_U + PELV_N_JOINTS + nForces;
  int nEqCon = PELV_N_SDFAST_U;
  ...
  qdot << Eigen::Map<const Eigen::Matrix<double,6,1> >(rs.rootd),
          Eigen::Map<const Eigen::Matrix<double,PELV_N_JOINTS,1> >(rs.jointsd);
  qdot *= QD_JD_USE_FRAC;
  ...
  */
  ...
  A.block<3,PELV_N_SDFAST_U>(rowIdx,0) = rs.Jpelvis.block<PELV_N_SDFAST_U,3>(0,3).transpose() * ANGDD_P_WEIGHT;
  b.segment<3>(rowIdx) += -rs.Jpelvisd.block<PELV_N_SDFAST_U,3>(0,3).transpose()*qdot;
   ...
   * bD = kp(q* - q) + kd(qd* - qd);
   ****************************/
  b.segment<PELV_N_JOINTS>(rowIdx) = Eigen::Map <const Eigen::Matrix<double,PELV_N_JOINTS,1> > (cmd.jointsdd);
  int pelvIdx;
  ...
  A.block(rowIdx,6,PELV_N_JOINTS,PELV_N_JOINTS) *= QDD_REG_WEIGHT;
  b.segment<PELV_N_JOINTS>(rowIdx) *= QDD_REG_WEIGHT;
       ...
       std::numeric_limits<double>::infinity()).any()) && COMDD_XY_WEIGHT != 0) 
  {
    ...
    A.block<3,PELV_N_SDFAST_U>(rowIdx,0) *= COMDD_XY_WEIGHT;
  ...
  {
    A.block<3,PELV_N_SDFAST_U>(rowIdx,0) = rs.Jtorso.block<PELV_N_SDFAST_U,3>(0,3).transpose() * ANGDD_T_WEIGHT;
    b.segment<3>(rowIdx) += -rs.Jtorsod.block<PELV_N_SDFAST_U,3>(0,3).transpose()*qdot;
  ...
  for (int side = 0; side < LR; side ++) {
    ...
    A.block<6,PELV_N_SDFAST_U>(rowIdx,0) = Jf[side]->transpose() * FOOTDD_WEIGHT;
    b.segment<6>(rowIdx) += -Jf[side]->transpose()*qdot;
    b.segment<6>(rowIdx) *= FOOTDD_WEIGHT;

    // stance
    if (!isSwing(rs.contactState, side)) {
      A.block<6,PELV_N_SDFAST_U>(rowIdx,0) *= 5e1;
  ...
  b.segment<PELV_N_JOINTS>(rowIdx) *= TAU_REG_WEIGHT;
  assert(!(b.segment<PELV_N_JOINTS>(rowIdx).array() == std::numeric_limits<double>::infinity()).any());
  rowIdx += PELV_N_JOINTS;
  ...
  //if (DTAU_REG_WEIGHT != 0 && !_torqueOld.isZero()) {
    A.block<PELV_N_JOINTS,PELV_N_JOINTS>(rowIdx,torqueStart).setIdentity();
    b.segment<PELV_N_JOINTS>(rowIdx) = _torqueOld.segment<PELV_N_JOINTS>(0);
  ...
  ***********************************************/
  ...
  for (int j = 0; j < PELV_N_JOINTS; j++) {
    Transmission::getTorqueLimit(j, rs.joints[j], tauLim.data()+j, tauLim.data()+j+PELV_N_JOINTS);
  }
  ...
  */ 
  ...
  switch (rs.contactState) {
    ...
    case DSr:
    case DSc: {
      const int N_VARS = PELV_N_SDFAST_U + PELV_N_JOINTS + 12;
      const int N_EQ_CON = PELV_N_SDFAST_U;
      //const int N_INEQ_CON = 9*12/6;
      const int N_INEQ_CON = 9*12/6 + PELV_N_JOINTS*2;
      ...
      //CI << Eigen::Matrix<double,PELV_N_SDFAST_U+PELV_N_JOINTS,N_INEQ_CON>::Zero(),
      //      AF.transpose();
    ...
    case SSL: 
    case SSR: {
      const int N_VARS = PELV_N_SDFAST_U + PELV_N_JOINTS + 6;
      const int N_EQ_CON = PELV_N_SDFAST_U;
      //const int N_INEQ_CON = 9*6/6;
      const int N_INEQ_CON = 9*6/6 + 2*PELV_N_JOINTS;
      Eigen::Matrix<double,N_VARS,N_VARS> G = 
          A.topRows(rowIdx).transpose()*A.topRows(rowIdx);
      Eigen::Matrix<double,N_VARS,1> g0 = 
          -A.topRows(rowIdx).transpose()*b.topRows(rowIdx);
  ...
  // save old torque
  _torqueOld.segment<PELV_N_JOINTS>(0) = X.segment<PELV_N_JOINTS>(torqueStart);
  ...
  /////////////////////////////////////////////////////////////
  // process output
  Eigen::Matrix<double,PELV_N_SDFAST_U,1> id_acc = X.topRows(PELV_N_SDFAST_U);
  Eigen::Matrix<double,PELV_N_JOINTS,1> id_trq = X.segment<PELV_N_JOINTS>(torqueStart);
  ...
  dvec_copy(output.jointsdd_d, id_acc.data()+6, PELV_N_JOINTS);
  dvec_copy(output.trq_ff, id_trq.data(), PELV_N_JOINTS);
  ...
  id_torsoWd = rs.Jtorso.block<PELV_N_SDFAST_U,3>(0,3).transpose() * id_acc - rs.Jtorsod.block<PELV_N_SDFAST_U,3>(0,3).transpose()*qdot;
  
  Eigen::Map <Eigen::Vector3d> id_pelvWd(output.pelvWd_d);
  id_pelvWd = rs.Jpelvis.block<PELV_N_SDFAST_U,3>(0,3).transpose() * id_acc - rs.Jpelvisd.block<PELV_N_SDFAST_U,3>(0,3).transpose()*qdot;
...
void SFPelvIdCon::addToLog(BatchLogger &logger) {
